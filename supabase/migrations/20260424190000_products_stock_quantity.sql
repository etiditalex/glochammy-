-- Add product stock tracking and enforce stock limits during checkout order creation.

alter table public.products
  add column if not exists stock_quantity integer not null default 0 check (stock_quantity >= 0);

-- Keep existing products purchasable after rollout; admins can edit exact numbers in Product form.
...
begin
  ...
  loop
    ...
    select name, price_cents, currency, stock_quantity
      into v_name, v_unit, v_curr, v_stock
